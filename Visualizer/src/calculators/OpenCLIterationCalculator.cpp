...
void printError(const cl_program& program, const cl_device_id& device) { // Member function of the class
...
void calculateItersInternal(const std::vector<T_ComplexType>& points, std::vector<int>& iters, const unsigned int maxIter, const char* kernelFilename) {
	...
	if (err != CL_SUCCESS) { // Print build errors only if compilation fails
	...
	cl_int max_iter_kernel = maxIter;
	err = clSetKernelArg(
		kernel,
		2,
		sizeof(unsigned int),
	...
	size_t local_work_size[1] = { 1000 };    // Maximum work size is 1024
